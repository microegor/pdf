...
import { createDictionary, createName, createNumber, createString } from "../types.js";
...
describe("document navigation", () => {
  ...
  it("getDict helpers resolve references", () => {
    const inner = createDictionary(new Map([["Value", createNumber(99)]]));
    ...
    const dict2 = createDictionary(new Map([["Arr", { type: "array", items: [createNumber(1)] }]]));
    const fakeDoc: any = {
      objects: new Map(),
    ...
    expect(
      getDictDict(fakeDoc, createDictionary(new Map([["D", inner]])), "D")?.entries.get("Value"),
