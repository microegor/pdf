...
describe('parser hardening regressions', () => {
  ...
  it('bounds direct values in one indirect object graph', () => {
    ...
    expect(doc.diagnostics.some(diagnostic => diagnostic.code === 'max-object-values')).toBe(true);
  });
  ...
  it('reuses the parsed XRef-stream object without spending its value budget twice', () => {
    ...
    expect(doc.diagnostics.some(diagnostic => diagnostic.code === 'max-object-values')).toBe(false);
  });
...
describe('binary and diff regressions', () => {
  ...
  it('uses direct history event locators', () => {
    ...
    expect(getObjectVersionDescriptors(doc, 1, 0).every(d => Number.isInteger(d.eventIndex))).toBe(
      true
    );
