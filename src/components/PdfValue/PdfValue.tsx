...
export function PdfValue({ value, depth = 0, onReferenceClick }: Props) {
  if (depth > MAX_DEPTH) {
    return <span className={styles.depthLimit}>Max depth reached</span>;
  }
  ...
  switch (value.type) {
    ...
    case "dictionary": {
      if (value.entries.size === 0) {
        return <span className={styles.empty}>Empty dictionary</span>;
      }
      ...
      return (
        <div className={styles.stream}>
          ...
          <div className={styles.streamMeta}>
            ...
            <span>{value.data.length} bytes</span>
          </div>

          {value.data.length > 0 && (
            <div className={styles.streamPreview} title={bytesToHexPreview(value.data, 128)}>
              {bytesToHexPreview(value.data, 24)}
